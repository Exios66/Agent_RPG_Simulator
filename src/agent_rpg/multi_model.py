...
def set_router_model_if_reactive(scenario: ScenarioConfig, model_id: str | None = None) -> None:
    """If turn order is reactive, set router model from ``model_id`` or first agent's model.

    When ``model_id`` is passed (notebook / CLI after ``assign_models_to_agents``), always
    apply it — ``build_random_scenario`` may have already seeded a placeholder default.
    """
